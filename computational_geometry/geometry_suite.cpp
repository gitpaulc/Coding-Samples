...
void render()
{
  ...
  glBegin(GL_POINTS);
  {
    int sizPointArray = point_array.size();

    for (int i = 0; i < sizPointArray; ++i)
    {
      const point_2d<double>& P = point_array[i];
      glVertex2f(P.x, P.y);
      //P.print("\n");
    }
  }
  ...
  {
    int sizPointArray = convex_hull.size();

    for (int i = 0; i < sizPointArray; i++)
    {
      point_2d<double> P = convex_hull[i];
      glVertex2f(P.x, P.y);
      //P.print("\n");
    }
  }
